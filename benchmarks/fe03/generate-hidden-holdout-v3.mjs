...
const CANDIDATE_SHA="28451d3207a630ac53fed377c5a4a1e126ced87a";
const UA="FutureEditionHiddenHoldout/3.0 (+https://github.com/BACOUL/FUTURE-EDITION)";
...
function addCase(family,candidate,labels,evidence,{relations=null,claim=null,notes=null}={}){
  ...
  seen.add(key);
  const id="FE03-EXT-"+String(9701+cases.length).padStart(4,"0");
...
const active=await pubmedMany("2013:2016[pdat] AND Journal Article[pt] NOT Review[pt] NOT Retracted Publication[pt]",10,r=>r.status==="active"&&r.abstract.length>100);
...
const retracted=await pubmedMany("2013:2016[pdat] AND Retracted Publication[pt]",10,r=>r.status==="retracted");
...
const notices=await pubmedMany("2013:2016[pdat] AND (Retraction of Publication[pt] OR Retraction Notice[pt])",8,r=>r.status==="active");
...
for(let i=0;i<5;i++){
  ...
  addCase("invalid_identifier",{
    ...
    raw_value:doi?"10.99998/future-edition-hidden-v3-"+String(800+i):"99888"+String(200+i),
    origin:"benchmark_design"
  },{
    primary_source_resolvable:false,publication_status:"unresolved",critical_safety_case:true,
    expected_subject_scope:"unknown",expected_evidence_level:"unknown"
  },ev("benchmark_design",null,["deterministic_invalid_identifier_v3"]));
}

const animals=await pubmedMany("2013:2016[pdat] AND Animals[MeSH Terms] NOT Humans[MeSH Terms] AND Journal Article[pt]",12,r=>r.status==="active"&&meshScope(r)==="animal"&&r.abstract.length>100);
...
const phase2=await pubmedMany("2013:2016[pdat] AND Clinical Trial, Phase II[pt] AND Randomized Controlled Trial[pt]",16,r=>
...
const phase3=await pubmedMany("2013:2016[pdat] AND Clinical Trial, Phase III[pt]",14,r=>
...
const reviews=await pubmedMany("2013:2016[pdat] AND (Systematic Review[pt] OR Meta-Analysis[pt]) AND Humans[MeSH Terms]",12,r=>
...
const negatives=await pubmedMany('2013:2016[pdat] AND Randomized Controlled Trial[pt] NOT Clinical Trial, Phase I[pt] NOT Clinical Trial, Phase II[pt] AND ("no significant difference"[Title/Abstract] OR "did not improve"[Title/Abstract] OR "not significant"[Title/Abstract])',14,r=>
...
const dependent=await pubmedMany("2013:2016[pdat] AND Journal Article[pt]",12,r=>r.status==="active"&&Boolean(r.doi)&&r.abstract.length>100);
...
async function medrxivCases(){
  const out=[];
  for(const [a,b] of [["2024-01-01","2024-12-31"],["2025-01-01","2025-11-30"]]){
...
async function arxivCases(){
  const q='submittedDate:[201801010000 TO 201912312359] AND all:benchmark AND (cat:cs.AI OR cat:cs.LG OR cat:cs.CV)';
...
async function contradictionPairs(){
  ...
  for(const topic of topics){
    const ids=await pubmedSearch("2013:2016[pdat] AND Randomized Controlled Trial[pt] AND "+topic,80);
...
if(cases.length<50) throw new Error("hidden holdout v3 underfilled: "+cases.length);

const annotations=[];
for(const x of claimPool.filter(x=>x.exact_text&&x.exact_text.length<=240)){
  if(annotations.length>=24) break;
  annotations.push({
    id:"FE03-CLAIM-"+String(9701+annotations.length).padStart(4,"0"),
    ...
    notes:"Fresh hidden V3 annotation generated only after candidate-v3 freeze."
  });
}
if(annotations.length<15) throw new Error("hidden claim annotations v3 underfilled: "+annotations.length);

const familyCounts={};
for(const x of cases) familyCounts[x.case_family]=(familyCounts[x.case_family]??0)+1;
const bundle={
  protocol:"FE03-HIDDEN-HOLDOUT-v3",
  created_after_candidate_freeze:true,
  candidate_sha:CANDIDATE_SHA,
  freshness:{
    v1_holdout_reused:false,
    v2_holdout_reused:false,
    public_ids_excluded:true,
    pubmed_window:"2013-2016",
    diagnostic_pubmed_window:"2017-2022",
    v2_hidden_pubmed_window:"2008-2012",
    medrxiv_window:"2024-2025",
    v3_provider_diagnostic_rxiv_window:"2022-2023",
    v2_hidden_rxiv_window:"2020-2021",
    arxiv_window:"2018-2019",
    v2_hidden_arxiv_window:"2013-2014"
  },
  external_cases:cases,
  claim_annotations:annotations
};
await writeFile("hidden-holdout-v3.json",JSON.stringify(bundle,null,2)+"\n");
const sha=createHash("sha256").update(await readFile("hidden-holdout-v3.json")).digest("hex");
console.log("FE03_HIDDEN_V3_GENERATED|cases="+cases.length+"|claims="+annotations.length+"|families="+Object.keys(familyCounts).length+"|sha256="+sha);
console.log("FE03_HIDDEN_V3_FAMILY_COUNTS|"+Object.entries(familyCounts).sort().map(([k,v])=>k+"="+v).join("|"));
