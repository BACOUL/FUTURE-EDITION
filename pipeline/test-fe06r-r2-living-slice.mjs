...
if((result.metrics?.selected??0)<5) throw new Error("R2 requires at least 5 resolved selected candidates");
if((result.metrics?.questions_covered??0)<3) throw new Error("R2 requires at least 3 questions covered");
...
console.log("FE06R_R2_LIVING_SLICE_PASS|selected="+result.metrics.selected+"|questions="+result.metrics.questions_covered+"|technology_questions="+result.metrics.technology_questions_covered+"|deduped="+result.metrics.deduplicated_resolved+"|candidate_only=1");
